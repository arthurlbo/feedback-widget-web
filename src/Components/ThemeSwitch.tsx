...
export function ThemeSwitch() {
    ...
    return (
        <button
            type="button"
            className="w-8 h-8 my-auto ml-4 rounded-full bg-zinc-400 hover:bg-zinc-500 dark:bg-zinc-700 dark:hover:bg-zinc-600 flex justify-center items-center "
            onClick={() => setIsDarkTheme((s) => !s)}
        >
            {isDarkTheme ? (
                <Sun className="h-5 w-5 " weight="fill" />
            ) : (
                <MoonStars className="h-5 w-5" weight="fill" />
            )}
        </button>
    );
}
